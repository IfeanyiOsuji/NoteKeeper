...
public class DataManager {
   ...
   public static void loadFromDataBase(NoteKeeperOpenHelper dbHelper){
        ...
        Cursor courseCursor = db.query(CourseInfoEntry.TABLE_NAME, courseColumns, null, null,
        ...
        String noteOrderBy = UpdateNoteInfoEntry.COLUMN_COURSE_ID +","+UpdateNoteInfoEntry.COLUMN_NOTE_TITLE;

       final Cursor noteCursor1 = db.query(UpdateNoteInfoEntry.TABLE_NAME1, noteColumns, null, null,
                null, null, noteOrderBy);
        loadNotesFromDatabase(noteCursor1);
       // noteCursor1.close();
    }

    private static void loadNotesFromDatabase(Cursor cursor) {
        int noteTitlePos = cursor.getColumnIndex(UpdateNoteInfoEntry.COLUMN_NOTE_TITLE);
        int noteTextPos = cursor.getColumnIndex(UpdateNoteInfoEntry.COLUMN_NOTE_TEXT);
        int courseIdPos = cursor.getColumnIndex(UpdateNoteInfoEntry.COLUMN_COURSE_ID);
        int idPos = cursor.getColumnIndex(UpdateNoteInfoEntry._ID);
